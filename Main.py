...
class SuperPipe(Frame):
	def __init__(self, parent):
		...
		Frame.__init__(self, parent, bg = self.theme.main_color)
	...
	def initUI(self):
		self.parent["bg"] = self.theme.main_color
		...
		top_left_side_bar = Frame(pw_side_bar, bg = self.theme.main_color)

		add_buttons = Frame(top_left_side_bar, bg = self.theme.main_color)
		add_buttons.pack(fill = X, pady = 10)

		add_buttons.columnconfigure(0, weight = 1)
		add_buttons.columnconfigure(1, weight = 1)

		self.add_asset_button = Button(add_buttons, text = "Add asset", state = DISABLED, bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 8, height = 1, command = self.addAssetCommand)
		self.add_asset_button.grid(row = 0, column = 0, sticky = N)

		self.add_shot_button = Button(add_buttons, text = "Add shot", state = DISABLED, bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 8, height = 1, command = self.addShotCommand)
		self.add_shot_button.grid(row = 0, column = 1, sticky = N)

		## ASSETS LIST ##
		asset_label = Label(top_left_side_bar, text = "Assets", bg = self.theme.main_color, fg = self.theme.text_color, font = "Helvetica 10 bold")
		asset_label.pack(fill = X, pady = 10)

		self.var_asset_filter = StringVar()
		self.asset_filter_entry = Entry(top_left_side_bar, textvariable = self.var_asset_filter, relief = FLAT, bg = self.theme.list_color, fg = self.theme.text_color, width = 50, bd = 5, validate = "key", validatecommand = (self.register(self.validateAssetFilterEntry), '%P'))
		...
		self.asset_list.tag_configure("done", background = self.theme.done_color)
		self.asset_list.tag_configure("urgent", background = self.theme.urgent_color)
		self.asset_list.tag_configure("high", background = self.theme.high_color)
		self.asset_list.tag_configure("medium", background = self.theme.medium_color)
		...
		bottom_left_side_bar = Frame(pw_side_bar, bg = self.theme.main_color)

		## SHOTS LIST ##
		shot_label = Label(bottom_left_side_bar, text = "Shots", bg = self.theme.main_color, fg = self.theme.text_color, font = "Helvetica 10 bold")
		shot_label.pack(fill = X, pady = 10)

		self.shot_list = Listbox(bottom_left_side_bar, bg = self.theme.list_color, selectbackground = self.theme.second_color, bd = 0, highlightthickness = 0, width = 30, exportselection = False)
		self.shot_list.pack(fill = BOTH, expand = Y)
		self.shot_list.bind("<<ListboxSelect>>", self.shotlistCommand)

		self.shots_preview_button = Button(bottom_left_side_bar, text = "Shots preview", state = DISABLED, bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 12, height = 1, command = self.shotsPreviewCommand)
		self.shots_preview_button.pack(pady = 10)

		self.custom_button = Button(bottom_left_side_bar, text = "Custom link", state = DISABLED, bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 12, height = 1, command = self.customButtonCommand)
		...
		main_area = Frame(pw_main, bg = self.theme.main_color)

		main_area.columnconfigure(0, weight = 1)

		## // NO PROJECT MAIN FRAME \\##
		self.main_area_home_page = Frame(main_area, bg = self.theme.main_color, bd = 0)
		...
		home_page_label = Label(self.main_area_home_page, textvariable = self.var_home_page_title, bg = self.theme.main_color, fg = self.theme.text_color, height = 1, justify = CENTER, font = "Helvetica 15 bold")
		home_page_label.pack(expand = True, fill = BOTH, side = BOTTOM)

		###############################################################################################################

		## // SHOTS MAIN FRAME \\ ##
		self.main_area_shot = Frame(main_area, bg = self.theme.main_color, bd = 0)
		...
		## SHOT INFOS ##
		self.up_down_shot = Frame(self.main_area_shot, bg = self.theme.main_color, bd = 0)
		...
		self.up_button = Button(self.up_down_shot, image = self.up_button_img, compound = "left", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, command = self.moveShotDownCommand)
		...
		self.down_button = Button(self.up_down_shot, image = self.down_button_img, compound = "left", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, command = self.moveShotUpCommand)
		...
		shot_nb_label = Label(self.main_area_shot, textvariable = self.var_shot_nb_label, bg = self.theme.main_color, fg = self.theme.text_color, height = 1, anchor = NW, font = "Helvetica 11 bold")
		shot_nb_label.grid(row = 0, column = 1)

		self.delete_shot_button_img = PhotoImage(file = "img/red_cross.gif")
		self.delete_shot_button = Button(self.main_area_shot, image = self.delete_shot_button_img, bg = self.theme.main_color, activebackground = self.theme.main_color, cursor = "hand2", fg = self.theme.text_color, bd = 0, command = self.deleteShotCommand)
		...
		self.set_shot_button = Button(self.main_area_shot, text = "Set shot", bg = self.theme.button_color1, activebackground = self.theme.over_button_color1, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 8, height = 1, command = self.setShotCommand)
		...
		self.frame_range_entry = Entry(self.main_area_shot, relief = FLAT, bg = self.theme.button_color2, bd = 5, width = 6, justify = CENTER, validate = "key", validatecommand = (self.register(self.validateFrameRangeEntry), '%P', '%S'))
		...
		self.set_shot_frame_range_button = Button(self.main_area_shot, text = "Set frame range", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 13, height = 1, command = self.setShotFrameRangeCommand)
		...
		self.open_shot_folder_button = Button(self.main_area_shot, text = "Open shot folder", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, fg = self.theme.text_color, activeforeground = self.theme.text_color, bd = 0, width = 16, height = 1, command = self.openFolderCommand)
		...
		shot_show_last_only_button = Checkbutton(self.main_area_shot, text = "Show only last versions", variable = self.var_check_show_last, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleLastVersions)
		shot_show_last_only_button.grid(row = 0, column = 6, sticky = E)

		## SHOT DESCRIPTION ##
		self.shot_description_line = Frame(self.main_area_shot, bg = self.theme.second_color, bd = 0)
		...
		shot_description_label = Label(self.shot_description_line, text = "Shot description :", bg = self.theme.second_color, fg = self.theme.text_color, height = 1, pady = 10, anchor = NW, font = "Helvetica 9 bold")
		shot_description_label.grid(row = 0, column = 1)

		self.var_shot_description = StringVar()
		self.shot_description_entry = Entry(self.shot_description_line, textvariable = self.var_shot_description, state = "readonly", readonlybackground = self.theme.main_color, relief = FLAT, bg = self.theme.main_color, fg = self.theme.text_color, width = 750, bd = 5, validate = "key", validatecommand = (self.register(self.validateDescriptionEntry), '%P'))
		self.shot_description_entry.grid(row = 1, column = 1, sticky = N, pady = (0, 20), padx = 100)

		## SHOT STATE ##
		self.shot_state_line = Frame(self.main_area_shot, bg = self.theme.main_color, bd = 0)
		...
		self.priority_shot_label = Label(self.shot_state_line, text = "Priority : ", bg = self.theme.main_color, fg = self.theme.text_color, height = 1, anchor = NW, font = "Helvetica 9 bold")
		...
		self.priority_shot_menu.config(bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, bd = 0, width = 10, highlightthickness = 0)
		...
		self.downgrade_shot_button = Button(self.shot_state_line, text = "Downgrade shot", state = DISABLED, bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 14, height = 1, command = self.downgradeShotCommand)
		...
		self.step_slider = CustomSlider(self.shot_state_line, width = 500, height = 20, steps = ("Layout", "Blocking", "Splining", "Rendering"), bg = self.theme.button_color2, fg = self.theme.button_color1, txt = self.theme.text_color, grid = self.theme.separator_color)
		...
		self.upgrade_shot_button = Button(self.shot_state_line, text = "Upgrade shot", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 13, height = 1, command = self.upgradeShotCommand)
		...
		self.done_shot_button = Checkbutton(self.shot_state_line, text = "Shot done", variable = self.var_shot_done, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleShotDone)
		...
		## SHOT PICTURES ##
		self.pictures_shot = Frame(self.main_area_shot, bg = self.theme.second_color, bd = 0)
		...
		self.prev_pict_label = Label(self.pictures_shot, text = "Previous shot", bg = self.theme.second_color, fg = self.theme.text_color, height = 1, anchor = N, font = "Helvetica 11")
		self.prev_pict_label.grid(row = 0, column = 0, pady = 10)
		self.prev_pict_label.pi = self.prev_pict_label.grid_info()

		self.shot_prev_pict_caneva = Canvas(self.pictures_shot, bg = self.theme.second_color, bd = 0, highlightthickness = 0)
		...
		this_pict_label = Label(self.pictures_shot, text = "This shot", bg = self.theme.second_color, fg = self.theme.text_color, height = 1, anchor = N, font = "Helvetica 11")
		this_pict_label.grid(row = 0, column = 1, pady = 10)

		self.shot_pict_caneva = Canvas(self.pictures_shot, bg = self.theme.second_color, bd = 0, highlightthickness = 0)
		...
		self.playblast_pictures_shot = Frame(self.main_area_shot, bg = self.theme.second_color, bd = 0)
		...
		playblast_pict_label = Label(self.playblast_pictures_shot, text = "This playblast", bg = self.theme.second_color, fg = self.theme.text_color, height = 1, anchor = N, font = "Helvetica 11")
		playblast_pict_label.grid(row = 0, column = 0, pady = 10)

		self.playblast_player = CustomVideoPlayer(self.playblast_pictures_shot, "test.mov", 512, self.theme.list_color)
		self.playblast_player.grid(row = 1, column = 0, pady = 20)

		## SHOT VERSION ACTIONS ##
		self.shot_actions_line = Frame(self.main_area_shot, bg = self.theme.main_color, bd = 0)
		...
		self.open_shot_button = Button(self.shot_actions_line, text = "Open selected version ", bg = self.theme.button_color1, activebackground = self.theme.over_button_color1, fg = self.theme.text_color, activeforeground = self.theme.text_color, bd = 0, width = 19, height = 1, command = self.openShotCommand)
		...
		## SHOT VERSION INFOS ##
		self.shot_version_management_line = Frame(self.main_area_shot, bg = self.theme.second_color, bd = 0)
		...
		shot_version_comment_label = Label(self.shot_version_management_line, text = "Version comment :", bg = self.theme.second_color, fg = self.theme.text_color, height = 1, pady = 10, anchor = NW, font = "Helvetica 9 bold")
		...
		shot_version_comment = Message(self.shot_version_management_line, textvariable = self.var_shot_version_comment, bg = self.theme.second_color, fg = self.theme.text_color, pady = 10, width = 750)
		shot_version_comment.grid(row = 1, column = 1, sticky = N)

		## PATHS ##
		self.shot_paths_line = Frame(self.main_area_shot, bg = self.theme.main_color, bd = 0)
		...
		shot_path_label = Entry(self.shot_paths_line, textvariable = self.var_selection_path_label, relief = FLAT, state = "readonly", readonlybackground = self.theme.main_color, fg = self.theme.text_color)
		shot_path_label.grid(row = 0, column = 0, sticky = W + E, pady = [0, 5])

		self.var_selection_abc_path_label = StringVar()
		shot_abc_path_label = Entry(self.shot_paths_line, textvariable = self.var_selection_abc_path_label, relief = FLAT, state = "readonly", readonlybackground = self.theme.main_color, fg = self.theme.text_color)
		shot_abc_path_label.grid(row = 1, column = 0, sticky = W + E, pady = 5)

		###############################################################################################################

		## // ASSETS MAIN FRAME \\ ##
		self.main_area_asset = Frame(main_area, bg = self.theme.main_color, bd = 0)
		...
		shot_nb_label = Label(self.main_area_asset, textvariable = self.var_asset_label, bg = self.theme.main_color, fg = self.theme.text_color, height = 1, anchor = NW, font = "Helvetica 11 bold")
		shot_nb_label.grid(row = 0, column = 1)

		self.delete_asset_button_img = PhotoImage(file = "img/red_cross.gif")
		self.delete_asset_button = Button(self.main_area_asset, image = self.delete_asset_button_img, bg = self.theme.main_color, activebackground = self.theme.main_color, fg = self.theme.text_color, cursor = "hand2", bd = 0, command = self.deleteAssetCommand)
		...
		self.rename_asset_button = Button(self.main_area_asset, text = "Rename asset", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 12, height = 1, command = self.renameAssetCommand)
		...
		self.set_asset_button = Button(self.main_area_asset, text = "Set asset", bg = self.theme.button_color1, activebackground = self.theme.over_button_color1, activeforeground = self.theme.text_color, fg = self.theme.text_color, bd = 0, width = 8, height = 1, command = self.setAssetCommand)
		...
		self.open_asset_folder_button = Button(self.main_area_asset, text = "Open asset folder", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, fg = self.theme.text_color, activeforeground = self.theme.text_color, bd = 0, width = 16, height = 1, command = self.openFolderCommand)
		...
		asset_show_last_only_button = Checkbutton(self.main_area_asset, text = "Show only last versions", variable = self.var_check_show_last, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleLastVersions)
		asset_show_last_only_button.grid(row = 0, column = 6, sticky = E)

		## ASSET STATE ##
		self.asset_state_line = Frame(self.main_area_asset, bg = self.theme.main_color, bd = 0)
		...
		self.priority_asset_label = Label(self.asset_state_line, text = "Priority : ", bg = self.theme.main_color, fg = self.theme.text_color, height = 1, anchor = NW, font = "Helvetica 9 bold")
		...
		self.priority_asset_menu.config(bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, activeforeground = self.theme.text_color, bd = 0, width = 10, highlightthickness = 0)
		...
		self.modeling_done_asset_button = Checkbutton(self.asset_state_line, text = "Modeling done", variable = self.var_asset_modeling_done, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetModelingDone)
		...
		self.rig_done_asset_button = Checkbutton(self.asset_state_line, text = "Rig done", variable = self.var_asset_rig_done, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetRigDone)
		...
		self.lookdev_done_asset_button = Checkbutton(self.asset_state_line, text = "Lookdev done", variable = self.var_asset_lookdev_done, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetLookdevDone)
		...
		self.done_asset_button = Checkbutton(self.asset_state_line, text = "Asset done", variable = self.var_asset_done, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetDone)
		...
		## ASSET PICTURES ##
		pictures_asset = Frame(self.main_area_asset, bg = self.theme.second_color, bd = 0)
		...
		pict_label = Label(pictures_asset, text = "This asset", bg = self.theme.second_color, fg = self.theme.text_color, height = 1, anchor = N, font = "Helvetica 11")
		pict_label.grid(row = 0, column = 0, pady = 10)

		self.asset_pict_caneva = Canvas(pictures_asset, bg = self.theme.second_color, bd = 0, highlightthickness = 0)
		...
		self.asset_actions_line = Frame(self.main_area_asset, bg = self.theme.main_color, bd = 0)
		...
		self.open_asset_button = Button(self.asset_actions_line, text = "Open selected version", bg = self.theme.button_color1, activebackground = self.theme.over_button_color1, fg = self.theme.text_color, activeforeground = self.theme.text_color, bd = 0, width = 19, height = 1, command = self.openAssetCommand)
		...
		## ASSET VERSION INFOS ##
		self.asset_version_management_line = Frame(self.main_area_asset, bg = self.theme.second_color, bd = 0)
		...
		shot_version_comment_label = Label(self.asset_version_management_line, text = "Version comment :", bg = self.theme.second_color, fg = self.theme.text_color, height = 1, pady = 10, anchor = NW, font = "Helvetica 9 bold")
		shot_version_comment_label.grid(row = 0, column = 1)

		self.var_asset_version_comment_label = StringVar()
		self.var_asset_version_comment_label.set("No comment")
		asset_version_comment_label = Message(self.asset_version_management_line, textvariable = self.var_asset_version_comment_label, bg = self.theme.second_color, fg = self.theme.text_color, pady = 10, width = 750)
		asset_version_comment_label.grid(row = 1, column = 1, sticky = N)

		## PATHS ##
		self.asset_paths_line = Frame(self.main_area_asset, bg = self.theme.main_color, bd = 0)
		...
		asset_path_label = Entry(self.asset_paths_line, textvariable = self.var_selection_path_label, relief = FLAT, state = "readonly", readonlybackground = self.theme.main_color, fg = self.theme.text_color)
		asset_path_label.grid(row = 0, column = 0, sticky = W + E)

		###############################################################################################################

		## SHOTS PREVIEW ##
		self.main_area_preview = Frame(main_area, bg = self.theme.main_color, bd = 0, width = 100, height = 100)
		...
		self.preview_canva_scroll = Canvas(self.main_area_preview, bg = self.theme.main_color, bd = 0, highlightthickness = 0, yscrollincrement = 20)

		self.shots_preview_list = Frame(self.preview_canva_scroll, bg = self.theme.main_color, bd = 0, width = 1000, height = 300)
		...
		right_side_bar = Frame(pw_main, bg = self.theme.main_color)

		## VERSIONS ##
		self.toggle_versions_playblasts_button = Button(right_side_bar, text = "Show playblasts", bg = self.theme.button_color2, activebackground = self.theme.over_button_color2, fg = self.theme.text_color, activeforeground = self.theme.text_color, bd = 0, width = 19, height = 1, command = self.toggleVersionsPlayblastsCommand)
		self.toggle_versions_playblasts_button.pack(pady = 10)

		self.var_versions_label = StringVar()
		self.var_versions_label.set("Versions")

		versions_label = Label(right_side_bar, textvariable = self.var_versions_label, bg = self.theme.main_color, fg = self.theme.text_color, font = "Helvetica 10 bold", width = 200)
		versions_label.pack(fill = NONE, pady = 10)

		display_buttons = Frame(right_side_bar, bg = self.theme.main_color)
		display_buttons.pack(fill = X)

		self.asset_display_buttons = Frame(display_buttons, bg = self.theme.main_color)
		...
		self.display_modeling_asset_button = Checkbutton(self.asset_display_buttons, text = "Display modeling", variable = self.var_asset_display_modeling, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetDisplay)
		...
		self.display_rigging_asset_button = Checkbutton(self.asset_display_buttons, text = "Display rigging", variable = self.var_asset_display_rigging, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetDisplay)
		...
		self.display_lookdev_asset_button = Checkbutton(self.asset_display_buttons, text = "Display lookdev", variable = self.var_asset_display_lookdev, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetDisplay)
		...
		self.display_other_asset_button = Checkbutton(self.asset_display_buttons, text = "Display other", variable = self.var_asset_display_other, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleAssetDisplay)
		self.display_other_asset_button.grid(row = 1, column = 1, sticky = W)
		self.display_other_asset_button.select()

		self.shot_display_buttons = Frame(display_buttons, bg = self.theme.main_color)
		...
		self.display_layout_shot_button = Checkbutton(self.shot_display_buttons, text = "Display layout", variable = self.var_shot_display_layout, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleShotDisplay)
		...
		self.display_blocking_shot_button = Checkbutton(self.shot_display_buttons, text = "Display blocking", variable = self.var_shot_display_blocking, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleShotDisplay)
		...
		self.display_splining_shot_button = Checkbutton(self.shot_display_buttons, text = "Display splining", variable = self.var_shot_display_splining, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleShotDisplay)
		...
		self.display_other_shot_button = Checkbutton(self.shot_display_buttons, text = "Display other", variable = self.var_shot_display_other, bg = self.theme.main_color, activeforeground = self.theme.text_color, fg = self.theme.text_color, activebackground = self.theme.main_color, selectcolor = self.theme.second_color, command = self.toggleShotDisplay)
		self.display_other_shot_button.grid(row = 2, column = 0, sticky = W)
		self.display_other_shot_button.select()

		self.version_list = Listbox(right_side_bar, bg = self.theme.list_color, selectbackground = self.theme.second_color, bd = 0, highlightthickness = 0, width = 50, height = 70, exportselection = False)
