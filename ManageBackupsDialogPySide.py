...
class ManageBackupsDialogPySide(QDialog):
	def __init__(self, parent=None, project=None):
		super(ManageBackupsDialogPySide, self).__init__(parent=parent, f=Qt.WindowTitleHint|Qt.WindowSystemMenuHint)
	...
	def submitCommand(self):
		for backup_item in self.backup_list.selectedItems():
			if backup_item not in self.categories:
				if backup_item.parent().text(0) == "SHOTS":
